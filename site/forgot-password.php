...
if(isset($_REQUEST['btnReset'])){
  extract($_REQUEST);
  $c= user_check('email',$email);
  $m=barber_check('email',$email);
if(!empty($c)){
  $user = user_check('email',$email);
  $code= time().$user['email'];
  $codehash=password_hash($code,PASSWORD_DEFAULT);
  $time_code=time();
  update_code_user($user['id'],$codehash,$time_code);
  $user = user_check('email',$email);
}elseif(!empty($m)){
  ...
  $codehash=password_hash($code,PASSWORD_DEFAULT);
  $time_code=time();
  update_code_barber($user['id'],$codehash,$time_code);
  $user=barber_check('email',$email);
}else{
...
$body = '<p>Xin chào '.$user['name'].',</p>
          ...
          <a href="'.ROOT.'resetPass/?code='.$user['code'].'&email='.$user['email'].'">'.ROOT.'resetPass/?code='.$user['code'].'&email='.$user['email'].'</a>
...
if($mail->send() == false){
 ...
 $_SESSION['messages'] = 'Mã xác minh đã được gửi đến địa chỉ email  <p class="stadurt">' . $user['email'].'</p> Vui lòng kiểm tra hộp thư đến của bạn!';
}
